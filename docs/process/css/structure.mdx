...
- If your component uses other components (as nested selectors) it will always be the highest level of the nested components + 1. So for example:
...
```scss title="template/__default.scss"
.21torr {
	.modal {
		position: fixed;
		// ...
...
See the article for [code style / directory layouts](/docs/process/code-style/index.mdx#scss).
