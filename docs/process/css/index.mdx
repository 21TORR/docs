...
All docs about the general structure are in its own article, under [Structure](structure).
...
The name of the file should exactly match the top level selector.
...
```scss title="helper/_mixins.scss"
$tablet-width: 90rem;
...
```scss title="molecule/_my-component.scss"
.my-component {
	// ...
