...
```shell
# This command should be run in the CI
bin/console hosting:validate-app
...
You can also view the build info on the CLI:

```shell
bin/console hosting:build:info
```
...
You can do your custom checks and mark the app as invalid. Please note that your message when marked as invalid is included in the API response, so it is public.
...
class MyHealthCheckListener
{
	...
	#[AsEventListener]
	public function onReadyCheck (HealthCheckReadyEvent $event) : void
	{
		// ...
